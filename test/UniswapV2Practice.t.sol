...
contract UniswapV2PracticeTest is Test {
    ...
    function test_maker_addLiquidityETH() public {
        // Implement here
        // maker approve to rounter
        vm.startPrank(maker);
        testUSDC.approve(address(UNISWAP_V2_ROUTER), 10000 * 10 ** testUSDC.decimals());
        // call addliquidity function in uniswap V2 router
    ...
    function test_taker_swapExactETHForTokens() public {
        ...
        // fee is reduce in input --> 1 eht to 0.997 eth (0.03% fee)
    ...
    function test_maker_removeLiquidityETH() public {
        ...
        console.log(WETHTestUSDCPair.balanceOf(maker));
        //999999999999000
        uint256 liq = WETHTestUSDCPair.balanceOf(maker);

        IUniswapV2Pair wethUsdcPair = IUniswapV2Pair(UNISWAP_V2_FACTORY.getPair(address(WETH9), address(testUSDC)));
        wethUsdcPair.approve(address(UNISWAP_V2_ROUTER), liq);
        // Implement here
        UNISWAP_V2_ROUTER.removeLiquidityETH(address(testUSDC), liq, 0, 0, maker, block.timestamp + 100);
        vm.stopPrank();
        // Checking
        // IUniswapV2Pair wethUsdcPair = IUniswapV2Pair(UNISWAP_V2_FACTORY.getPair(address(WETH9), address(testUSDC)));
        (uint112 reserve0, uint112 reserve1, ) = wethUsdcPair.getReserves();
